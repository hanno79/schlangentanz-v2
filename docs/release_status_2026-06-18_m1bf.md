...
Der finale Stand ist per Commit/Push auf `main` und Vercel Production auf die stabile Alias `https://schlangentanz-v2.vercel.app` ausgeliefert. Der Alias-Smoke `node scripts/live_smoke.mjs` bestätigt den M1bf-Vertrag: `M1bf Nachziehstapel: Deckobjekt vor Ablage mit 3px-Rand und Hard Shadow sichtbar`. Diese Datei hält bewusst den stabilen Alias statt ephemerer Deploy-URLs fest.

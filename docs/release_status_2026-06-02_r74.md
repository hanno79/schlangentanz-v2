...
- Tests: `src/engine/__tests__/turn_state.test.ts` prüft das gezielte Aussetzen beim Zugwechsel; `src/engine/__tests__/legal_actions.test.ts` prüft die auswählbaren Zielspieler.
- Verifikation:
  - `npm test -- --run src/engine/__tests__/legal_actions.test.ts src/engine/__tests__/turn_state.test.ts src/App.r53.test.tsx` ✅
  - `npm test -- --run` ✅ — 50 Testfiles, 297 Tests
  - `npm run typecheck` ✅
  - `npm run lint` ✅
  - `npm run build` ✅

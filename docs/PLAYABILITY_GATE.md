...
- [x] Commit/Push/Deploy/Smoke: Feature-Commit `0e2b621 — M1bl: Spieltisch als Waldtanz-Buehne fokussieren` wurde nach `origin/main` gepusht und per Vercel Production auf die stabile Alias `https://schlangentanz-v2.vercel.app` bereitgestellt (`READY`, 17s). Production-Smoke bestätigt `/` und `/game` HTTP 200, Kernregionen, bestehende M1as/M1aw/M1ax/M1ay/M1bc/M1bd/M1bf/M1bg/M1bi/M1bj/M1ba/M1bb/M1bk-Verträge und neu `M1bl Bühnenrahmen`; keine Console-/Page-Errors. Der Dokumentations-HEAD wird anschließend erneut deployt und gesmoked, damit Production und `origin/main` übereinstimmen.

...
- [x] Commit/Push/Deploy/Smoke: Feature-Commit `670811e` wurde nach `origin/main` gepusht und per Vercel Production auf `https://schlangentanz-v2.vercel.app` bereitgestellt (`https://schlangentanz-v2-be0xpqvob-alfreds-projects-7e9df1b4.vercel.app` aliasiert). Smoke: `npm run smoke:production` grün mit `/game` und `/` HTTP 200; M5d-Browser-Smoke startet 3-KI-Lobby, bestätigt `Zugkompass` zwischen `Zugpfad` und `Schlangenbereich`, 3px Dark-Forest-Border / hard shadow / radiale Waldlichtung, spielt board-nah eine Startkarte, führt per Zugkompass zur Aufgabenprüfung und zum Zugabschluss, übergibt an KI, spult Gegnerzüge zurück zum Menschen und meldet keine Console-/Page-Errors.

...
- [x] Commit/Push/Deploy/Smoke: Nach Commit/Push wird der finale `origin/main`-HEAD per Vercel Production auf die stabile Alias `https://schlangentanz-v2.vercel.app` bereitgestellt; `npm run smoke:production` enthält M1cp dauerhaft und wird final auf der Alias gesmoked.

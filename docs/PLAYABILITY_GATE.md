...
- [x] Deploy/Smoke: Feature-Commit auf Production-Alias bereitgestellt; finaler Doku-/Smoke-Skript-HEAD wird erneut deployt. Production-Smoke: `/` und `/game` HTTP 200, `npm run smoke:production` grün, Playwright klickt ein sichtbares `Schlangenbereich-Start`-Boardziel, sieht `Zuletzt ausgeführt: Neue Schlange starten`, eine neue eigene Schlange, `dockPointerEvents: none`, `dockButtonPointerEvents: auto`, `boardPosition: relative`, `boardZIndex: 3`, `startScrollMarginBottom: 324px`, keine Console-/Page-Errors.

...
describe('M1ae Waldtanz-Erstbild', () => {
  it('ordnet auf /game Spielrahmen, Arenastein, Zugrail und Handkarten als kompaktes erstes Brettbild statt langer Vorlauf-Liste an', () => {
    ...
    const partiefortschritt = within(spieltisch).getByRole('region', { name: 'Partiefortschritt' })
    ...
    expect(cssBlock('.spielbereich--game-route [class~="waldtanz-arenastein"]')).toMatch(/grid-row:\s*3\s*\/\s*span\s*5/)
    expect(cssBlock('.spielbereich--game-route [class~="handkarten-panel"]')).toMatch(/grid-column:\s*1\s*\/\s*-1/)
    expect(cssBlock('.spielbereich--game-route [class~="zugkompass"]')).toMatch(/grid-column:\s*2/)
    expect(cssBlock('.spielbereich--game-route [class~="partiefortschritt"]')).toMatch(/grid-column:\s*2/)

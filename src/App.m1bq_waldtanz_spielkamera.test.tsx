...
describe('M1bq Waldtanz-Spielkamera', () => {
  ...
  it('legt den CSS- und Browser-Smoke-Vertrag fuer eine breite Stitch-Brettkamera ab', () => {
    ...
    expect(routeGrid).toMatch(/grid-template-columns:\s*minmax\(9\.5rem,\s*0\.34fr\)\s*minmax\(0,\s*2\.66fr\)/)
    expect(routeGrid).toMatch(/gap:\s*clamp\(0\.65rem,\s*1vw,\s*0\.9rem\)/)
    ...
    expect(routeBrett).toMatch(/grid-template-columns:\s*minmax\(0,\s*1\.9fr\)\s*minmax\(14rem,\s*0\.58fr\)/)
    expect(routeWaldstein).toMatch(/max-height:\s*clamp\(26rem,\s*60vh,\s*34rem\)/)
    expect(routeZugleiste).toMatch(/max-height:\s*clamp\(26rem,\s*60vh,\s*34rem\)/)
    expect(smokeScript).toContain('M1bq Spielkamera')
    expect(smokeScript).toContain('brettWidth < 980')
    expect(smokeScript).toContain('waldsteinWidth < 660')
    expect(smokeScript).toContain('seitenrahmenWidth > 190')
  })
})

...
describe('M1bt Waldtanz-Startlichtung', () => {
  ...
  it('legt CSS- und Browser-Smoke-Vertrag ab, damit der Startkreis nicht von der Handbank überdeckt wird', () => {
    ...
    expect(appCss).toMatch(/@media\s*\(min-width:\s*1100px\)\s*\{[\s\S]*\.spielbereich--game-route \[class~="schlangenbereich--waldlichtung"\] \[class~="schlangen-gruppe"\]:first-of-type\s*\{[\s\S]*transform:\s*translateY\(-6\.1rem\)/)
    expect(routeEigene).toMatch(/transform:\s*translateY\(-6\.1rem\)/)
    ...
    expect(smokeScript).toContain('metrics.startBottom > metrics.handTop - 12')

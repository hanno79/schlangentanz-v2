...
describe('M1as Waldtanz-Erstzug-Lichtung', () => {
  ...
  it('kompaktiert die /game-Lichtung, damit Startkreis und Hand ohne innere Scrollsuche im Erstbild liegen', () => {
    ...
    expect(cssBlock('.spielbereich--game-route [class~="waldtanz-magiekreise__liste"]')).toMatch(/grid-template-columns:\s*repeat\(3,\s*minmax\(0,\s*1fr\)\)/)
    expect(cssBlock('.spielbereich--game-route [class~="waldtanz-magiekreise__kreis"]')).toMatch(/min-height:\s*4rem/)
    expect(cssBlock('.spielbereich--game-route [class~="waldtanz-tischkarte"]')).toMatch(/width:\s*min\(100%,\s*22rem\)/)
    expect(cssBlock('.spielbereich--game-route [class~="waldtanz-tischkarte"]')).toMatch(/grid-column:\s*1/)
    expect(cssBlock('.spielbereich--game-route [class~="waldtanz-magiekreise"]')).toMatch(/grid-column:\s*1/)
    expect(cssBlock('.spielbereich--game-route [class~="waldtanz-magiekreise"]')).toMatch(/grid-row:\s*2/)
    expect(cssBlock('.spielbereich--game-route [class~="schlangenbereich--waldlichtung"]')).toMatch(/min-height:\s*7\.5rem/)
    expect(cssBlock('.spielbereich--game-route [class~="schlangenbereich--waldlichtung"]')).toMatch(/grid-column:\s*2/)
    expect(cssBlock('.spielbereich--game-route [class~="schlangenbereich--waldlichtung"]')).toMatch(/grid-row:\s*1\s*\/\s*span 2/)

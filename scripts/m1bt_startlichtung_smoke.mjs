...
async function main() {
  ...
  try {
    ...
    if (metrics.startBottom > metrics.handTop - 4) throw new Error(`M1bt Startlichtung: Startkreis wird von der Handbank ueberdeckt ${JSON.stringify(metrics)}`)
    ...
    if (!metrics.hitClass.includes('schlangen-startzone--magiekreis')) throw new Error(`M1bt Startlichtung: Pruefpunkt nicht hit-testbar ${JSON.stringify(metrics)}`)
